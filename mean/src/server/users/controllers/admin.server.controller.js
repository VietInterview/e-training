...
exports.update = function(req, res) {
  ...
  user.permissionApi = req.body.permissionApi;
  ...
  async.waterfall(
    [
          ...
          if (!err && setting && setting.valueBoolean) {
            User.find({
              ...
              _.each(users, function(recipient) {
                var alert = new Message({
                  title: 'User account',
                  content: 'User ' + user.username + ' has been deleted',
                  level: 'danger',
                  type: 'alert',
                  recipient: recipient._id
                });
      ...
      function(user, done) {
        ...
        var baseUrl = req.app.get('domain') || httpTransport + req.headers.host;
        res.render(path.resolve('src/server/users/templates/user-removal-notification-email'), {
          name: user.displayName,
          appName: config.app.title
        }, function(err, emailHTML) {
          done(err, emailHTML, user);
