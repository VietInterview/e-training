    ...
    return {
      ...
      link: function(scope, element, attributes) {
          ...
          UIkit.modal.prompt($translate.instant('MODEL.GROUP.NAME'), '', function(val) {
            ...
            group.$save(function(response) {
              ...
              Notification.error({
                ...
                title: '<i class="uk-icon-ban"></i> Group created error!'
              });
            });
          });
        };
        ...
        scope.remove = function(node) {
          ...
          if (!node) {
            Notification.error({
              message: '<i class="uk-icon-ban"></i> No node selected!'
            });
            return;
          }
          if (node.children) {
            Notification.error({
              message: '<i class="uk-icon-ban"></i> Cannot remove non-empty node!'
            });
            return;
          }
          var group = node.data;
            ...
            AdminService.byGroup({
              ...
              if (users.length > 0) {
                ...
                group.$remove(function(response) {
                  Notification.success({
                    message: '<i class="uk-icon-ok"></i> Group removed successfully!'
                  });
                  reloadTree();
                }, function(errorResponse) {
                  Notification.error({
                    message: errorResponse.data.message,
                    title: '<i class="uk-icon-ban"></i> Group removed error!'
                  });
                });
            });
