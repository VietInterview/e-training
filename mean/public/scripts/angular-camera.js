  ...
  angular.module('omr.directives', []).directive('ngCamera', [
    ...
    function ($timeout, $sce) {
      return {
        ...
        '<video id="ng-camera-feed" ng-hide="overlaySrc" autoplay width="{{width}}" height="{{height}}" src="{{videoStream}}">Install Browser\'s latest version</video>' +
        '<canvas id="ng-photo-canvas" width="{{width}}" height="{{height}}" style="display:none;"></canvas>' +
        '</div>' +
        '<div class="ng-camera-controls" style="margin-top: 12px;" ng-hide="hideUI || overlaySrc">' +
        '<button class="uk-button uk-button-primary ng-camera-take-btn" ng-click="takePicture()">CHỤP</button>' +
        '</div>' +
        '</div>',
        replace: false,
        transclude: true,
